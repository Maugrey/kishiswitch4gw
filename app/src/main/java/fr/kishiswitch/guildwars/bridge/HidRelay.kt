...
internal class HidRelay(private val config: JSONObject, private val lifetime: IBinder) : AutoCloseable {
    companion object {
        ...
        private fun command(vararg args: String): String {
            ...
            try {
                check(process.waitFor(4,TimeUnit.SECONDS)) { "Identification du périphérique trop lente" }
                check(process.exitValue()==0) { "Identification refusée : ${read.get(1,TimeUnit.SECONDS).take(180)}" }
        ...
        private fun blockFor(descriptor:String):String {
            ...
            return headers.mapIndexed { i,m -> dump.substring(m.range.first,headers.getOrNull(i+1)?.range?.first ?: dump.length) }
                ...
                ?: error("Kishi physique introuvable")
        }
    }
    ...
    fun start() {
        ...
        Thread({
            ...
            if(thread.isAlive) {
                if(!stopped) { error="Connexion de contrôle expirée";state="failed" }
    ...
    private fun run() {
        ...
        try {
            ...
            val device=InputDevice.getDevice(id) ?: error("Kishi débranchée")
            check(device.descriptor==config.getString("descriptor") && device.vendorId==0x1532 && device.productId==0x0717) { "Ce relais nécessite la Kishi V2 Pro identifiée" }
            check(config.getInt("lt") in setOf(17,23) && config.getInt("rt") in setOf(18,22)) { "Axes des gâchettes incompatibles avec ce profil" }
            check(!requested.rightVertical || config.optInt("ry",14)==14) { "Ce profil attend AXIS_RZ" }
            val block=blockFor(device.descriptor)
            val path=Regex("(?m)^      Path: (/dev/input/event\\d+)\\s*$").find(block)?.groupValues?.get(1) ?: error("Chemin Kishi absent")
            check(block.contains("KeyState (pressed): <none>")) { "Relâchez les boutons puis revenez dans Guild Wars" }
            ...
            for((i,name) in names.withIndex()) {
                val m=Regex("$name\\s*: value (-?\\d+), min (-?\\d+), max (-?\\d+), fuzz \\d+, flat (\\d+)").find(capabilities)
                    ?: error("Axe physique absent : $name")
            ...
            check(Regex("ABS_[A-Z0-9]+\\s*:").findAll(capabilities).count()==8) { "Axes supplémentaires non pris en charge" }
            ...
            check(mapper.neutral) { "Relâchez les sticks et gâchettes puis revenez dans Guild Wars" }
            if(stopped) return
            ...
            check(InputDevice.getDeviceIds().any { InputDevice.getDevice(it)?.name==DEVICE_NAME }) { "Manette UHID non reconnue par Android" }
            check(hid.isAlive) { "La transmission HID s’est arrêtée" }
            ...
            check(beforeGrab.contains("Path: $path\n")) { "La Kishi a changé pendant la connexion" }
            check(beforeGrab.contains("KeyState (pressed): <none>")) { "Relâchez les boutons pendant la connexion puis réessayez" }
            for((i,name) in names.withIndex()) {
                val value=Regex("$name=(-?\\d+)").find(beforeGrab)?.groupValues?.get(1)?.toInt()
                    ?: error("État de la Kishi indisponible")
                check(if(i<4) kotlin.math.abs(value-128)<=flats[i] else value==0) { "Relâchez les commandes pendant la connexion puis réessayez" }
            }
            fd=Os.open(path,OsConstants.O_RDONLY or OsConstants.O_NONBLOCK,0)
            synchronized(this) { grabbedFd=fd;check(!stopped) { "Session terminée" } }
            ...
            while(!stopped && SystemClock.uptimeMillis()<lease && hid.isAlive) {
                ...
                if(Os.poll(arrayOf(poll),40)>0) {
                    check(poll.revents.toInt() and (OsConstants.POLLERR or OsConstants.POLLHUP or OsConstants.POLLNVAL)==0) { "Kishi débranchée" }
                    val count=Os.read(fd,raw,0,raw.size)
                    check(count>0 && count%24==0) { "Flux de manette incomplet" }
                    for(i in 0 until count step 24) {
                        ...
                        when(type) {
                            0 -> when(code){ 0 -> send(writer,mapper.frame()); 3 -> error("Commandes perdues : SYN_DROPPED") }
            ...
            if(!stopped && !drain) error("Connexion de contrôle interrompue")
            state="stopped"
        } catch(e:Exception) {
